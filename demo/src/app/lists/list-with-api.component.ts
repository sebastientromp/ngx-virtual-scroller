...
@Component({
  ...
  template: `
    <label
      ...
      <input
        type="checkbox"
        (change)="shouldPrependItems = !shouldPrependItems"
    /></label>
...
export class ListWithApiComponent extends BaseListComponent {
  ...
  public fetchMore(event: IPageInfo) {
    ...
    if (
      !this.shouldPrependItems &&
      event.endIndex === this.filteredList.length - 1
    ) {
      this.fetchNextChunk();
    }
  }
